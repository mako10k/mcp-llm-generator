# MCP LLM Generator v2 - 運用手順書 📋

> **目的**: MCPシステムの日常運用、メンテナンス、トラブルシューティングの包括的ガイド

## 📖 目次

1. [日常運用手順](#1-日常運用手順)
2. [開発・テスト手順](#2-開発テスト手順)
3. [デプロイメント手順](#3-デプロイメント手順)
4. [メンテナンス手順](#4-メンテナンス手順)
5. [トラブルシューティング](#5-トラブルシューティング)
6. [セキュリティ運用](#6-セキュリティ運用)
7. [監視・ログ管理](#7-監視ログ管理)
8. [緊急時対応](#8-緊急時対応)
...
# 5. ヘルスチェック
curl -s http://localhost:3000/health || echo "HTTP interface not available (stdio mode)"
```

#### 🛑 システム停止
```bash
# VS Code MCP管理の場合
# → VS Codeのサーバー管理UIから停止

# 直接プロセスの場合
# → Ctrl+C または SIGTERM シグナル

# 強制停止（緊急時のみ）
pkill -f "node.*mcp-llm-generator"
```
