...
### Feature #NEW-1: Context-Chat プロバイダー動的切り替え機能
- **要求背景**: GitHub Copilotプレミアムリクエスト節約とコスト最適化
- **要求仕様**:
  - Context-Chat実行時にプロバイダー（OpenAI/Claude等）を選択可能
  - 既存のコンテキスト・人格設定を維持しながらプロバイダーのみ変更
  - API使用量・コスト情報の表示機能
- **実装要件**:
  - `context-chat` MCPツールに `provider` パラメータ追加
  - プロバイダー別のトークン消費量・コスト追跡
  - フォールバック機能（第一プロバイダー失敗時の自動切り替え）
- **優先度**: P2 (ユーザビリティ向上)
- **推定工数**: 2-3日
- **依存関係**: 既存のexternal-llm-generate機能
