...
async function expandTemplate(templateName, args) {
...
server.registerResource("sample-config", "sample://config", {
...
// Templates resource
server.registerResource("templates", "sample://templates", {
...
server.registerResource("sample-result", new ResourceTemplate("sample://results/{id}", { list: undefined }), {
    title: "Sample Result",
...
server.registerResource("template-detail", new ResourceTemplate("sample://templates/{name}", { list: undefined }), {
...
server.registerTool("template-execute", {
...
server.registerTool("llm-generate", {
...
server.registerPrompt("sample-template", {
...
async function saveTemplatesToFile(templates) {
    try {
...
server.registerTool("template-manage", {
...
server.registerTool("template-to-params", {
...
server.registerTool("external-llm-generate", {
...
server.registerTool("external-llm-providers", {
...
server.registerTool("context-manage", {
    ...
    const request = { params: { name: "context-manage", arguments: args } };
...
server.registerTool("personality-preset-manage", {
    ...
    const request = { params: { name: "personality-preset-manage", arguments: args } };
    const result = await contextMemory.handleToolCall(request);
    return result || { content: [{ type: 'text', text: 'No response from personality-preset-manage tool' }] };
});
server.registerTool("context-chat", {
    title: "Context Chat",
    ...
    const request = { params: { name: "context-chat", arguments: args } };
    const result = await contextMemory.handleToolCall(request);
    return result || { content: [{ type: 'text', text: 'No response from context-chat tool' }] };
});
server.registerTool("conversation-manage", {
    ...
    const request = { params: { name: "conversation-manage", arguments: args } };
    const result = await contextMemory.handleToolCall(request);
...
async function main() {
    try {
        ...
        await contextMemory.initialize(server, async (messages, options) => {
