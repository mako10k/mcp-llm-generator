...
import { DEFAULT_VALUES } from '../types/index.js';
import { createContext, createContextFromPreset, updateContext, createConversation, createPersonalityPreset, updatePersonalityPreset, validateContextInput, validatePresetInput, isContextExpired, truncateConversationHistory } from '../utils/index.js';
// =============================================================================
...
export class ContextMemoryTools {
     ...
     */
    setCreateMessageCallback(callback) {
     ...
     */
    async handleContextManage(args) {
    ...
    handleContextCreateFromPreset(input) {
        ...
        try {
            const context = createContextFromPreset(input.presetId, input.name, input.presetOverrides, this.db // Pass database instance for dynamic preset lookup
            );
            const createdContext = this.db.createContext(context);
     ...
     */
    async handlePersonalityPresetManage(args) {
    ...
    async handleContextChat(args) {
        ...
        try {
            ...
            const { truncated: conversationHistory, wasTruncated } = truncateConversationHistory(allConversations, context.maxHistoryTokens);
     ...
     */
    async handleConversationManage(args) {
    ...
    handleConversationDelete(input) {
        let deletedCount = 0;
