...
import { DEFAULT_PERSONALITY_PRESETS } from '../types/index.js';
// =============================================================================
...
export class ContextMemoryDatabase {
    ...
    `);
        ...
        for (const [key, preset] of Object.entries(DEFAULT_PERSONALITY_PRESETS)) {
            insertPreset.run(preset.id, preset.name, preset.description, preset.systemPrompt, preset.defaultPersonality, preset.defaultSettings.temperature, preset.defaultSettings.maxTokens, preset.defaultSettings.maxHistoryTokens, preset.defaultSettings.expiryDays, now, now, 1, JSON.stringify({}));
        }
    }
    /**
    ...
    `);
        stmt.run(context.id, context.name, context.systemPrompt, context.personality, context.temperature, context.maxTokens, context.maxHistoryTokens, context.expiryDays, context.createdAt, context.updatedAt, context.expiresAt, context.isActive ? 1 : 0);
        return context;
    }
    /**
     * Get context by ID
    ...
    updateContext(context) {
      ...
      WHERE id = ?
    `);
        const result = stmt.run(context.name, context.systemPrompt, context.personality, context.temperature, context.maxTokens, context.maxHistoryTokens, context.expiryDays, context.updatedAt, context.expiresAt, context.isActive ? 1 : 0, context.id);
        if (result.changes === 0) {
    ...
    createConversation(conversation) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
        stmt.run(conversation.id, conversation.contextId, conversation.role, conversation.content, conversation.tokenCount, conversation.createdAt);
        return conversation;
    }
    /**
     * Get conversations for a context
     */
    getConversations(contextId, options = {}) {
        const page = options.page || 1;
    ...
    `);
        const { count: totalCount } = countStmt.get(contextId);
        // Get paginated results
        const offset = (page - 1) * pageSize;
        const listStmt = this.db.prepare(`
      ...
      LIMIT ? OFFSET ?
    `);
        const rows = listStmt.all(contextId, pageSize, offset);
     ...
     */
    getAllConversations(contextId) {
        const stmt = this.db.prepare(`
      SELECT * FROM conversations 
      WHERE context_id = ?
      ORDER BY created_at ASC
    `);
        const rows = stmt.all(contextId);
     ...
     */
    deleteConversationsOlderThan(contextId, olderThan) {
        const stmt = this.db.prepare(`
     ...
     */
    createPersonalityPreset(preset) {
        const stmt = this.db.prepare(`
    ...
    `);
        stmt.run(preset.id, preset.name, preset.description, preset.systemPrompt, preset.defaultPersonality, preset.defaultSettings.temperature, preset.defaultSettings.maxTokens, preset.defaultSettings.maxHistoryTokens, preset.defaultSettings.expiryDays, preset.createdAt, preset.updatedAt, preset.isActive ? 1 : 0, JSON.stringify(preset.metadata || {}));
        return preset;
    }
    /**
     * Get personality preset by ID
     */
    getPersonalityPreset(id) {
        const stmt = this.db.prepare(`
     ...
     */
    updatePersonalityPreset(preset) {
        const stmt = this.db.prepare(`
      ...
      WHERE id = ?
    `);
        const result = stmt.run(preset.name, preset.description, preset.systemPrompt, preset.defaultPersonality, preset.defaultSettings.temperature, preset.defaultSettings.maxTokens, preset.defaultSettings.maxHistoryTokens, preset.defaultSettings.expiryDays, preset.updatedAt, preset.isActive ? 1 : 0, JSON.stringify(preset.metadata || {}), preset.id);
        if (result.changes === 0) {
    ...
    getStatistics() {
      ...
      FROM contexts
    `);
        const contextStats = contextStatsStmt.get();
        ...
        const conversationStats = conversationStatsStmt.get();
    ...
    `);
        const presetStats = presetStatsStmt.get();
