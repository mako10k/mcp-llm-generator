...
export class SharedMemoryCore {
    ...
    createMemory(request) {
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `);
            stmt.run(memoryId, validatedData.title, validatedData.content, validatedData.creator_persona_id, validatedData.permission_level, validatedData.creator_persona_id);
            // 通知の作成
            this.insertNotification(memoryId, validatedData.title, 'created', // テスト仕様に合わせて修正
            'メモリが作成されました。', validatedData.creator_persona_id);
     ...
     * 共有メモの検索
     */
    searchMemories(request, requesterPersonaId) {
        try {
      ...
      `);
        ...
        LIMIT ? OFFSET ?
      `);
            const memories = selectStmt.all(...params, validatedParams.limit, validatedParams.offset);
            return {
                success: true,
                data: {
                    memories,
     ...
     * 共有メモの更新
     */
    updateMemory(request, updaterPersonaId) {
        try {
      ...
      `);
            ...
            // 通知の作成
            this.insertNotification(validatedData.id, validatedData.title || existingMemory.title, 'updated', `「${validatedData.title || existingMemory.title}」が更新されました`, updaterPersonaId);
            return { success: true };
        }
        catch (error) {
    ...
    deleteMemory(id, deleterPersonaId) {
        try {
            ...
            deleteStmt.run(id);
            // 通知の作成
            this.insertNotification(id, existingMemory.title, 'deleted', `「${existingMemory.title}」が削除されました`, deleterPersonaId);
            return { success: true };
        }
        catch (error) {
     ...
     * 通知の挿入（内部メソッド）
     */
    insertNotification(memoryId, memoryTitle, type, message, updatedBy) {
        try {
