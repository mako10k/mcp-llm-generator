...
import { FunctionExecutionEngine } from './functionExecutionEngine.js';
// =============================================================================
...
export class FunctionCallDispatcher {
     ...
     */
    async dispatchFunctionCall(request) {
        ...
        try {
            ...
            if (!validationResult.valid) {
                return this.createErrorResponse(request, requestId, `Validation failed: ${validationResult.reason}`, startTime);
            }
            // Step 2: PersonaManagerを通じたタスク委譲実行
            const delegationResult = await this.executeDelegatedFunction(request, requestId);
            if (!delegationResult.success) {
                return this.createErrorResponse(request, requestId, delegationResult.error || 'Function execution failed', startTime);
            }
            // Step 3: 成功レスポンスの構築
            const response = {
                success: true,
            ...
            this.logger.logDatabaseOperation(methodName, 'function_call_success', true, requestId, {
            ...
            this.logger.error('Function call dispatch failed', {
                method: methodName,
                contextId: requestId,
            ...
            return this.createErrorResponse(request, requestId, `Dispatch error: ${error.message}`, startTime);
        }
    }
    /**
     * Function実行権限・能力の検証
     */
    async validateFunctionCall(request) {
        ...
        // 権限チェック
        const hasPermission = this.personaManager.checkRolePermissions(request.fromPersonaId, 'function_call');
        if (!hasPermission) {
     ...
     */
    async executeDelegatedFunction(request, requestId) {
        ...
        try {
            ...
            this.logger.error('Function delegation failed', {
                method: methodName,
                contextId: requestId,
     ...
     * エラーレスポンスの構築
     */
    createErrorResponse(request, requestId, errorMessage, startTime) {
        return {
            success: false,
            error: errorMessage,
            logId: requestId,
     ...
     * Function Registry管理
     */
    registerPersonaFunctions(registry) {
     ...
     * 利用可能Function一覧の取得
     */
    getAvailableFunctions(personaId) {
