...
async function initializeDatabase() {
    ...
    try {
        ...
        const missingTables = requiredTables.filter(tableName => !tables.some(table => table.name === tableName));
...
async function insertSampleData(db) {
    ...
    `);
        sampleCapabilities.forEach(cap => {
            capabilityStmt.run(cap.context_id, JSON.stringify(cap.expertise), JSON.stringify(cap.tools), JSON.stringify(cap.restrictions), cap.is_public);
        });
        // サンプルロール
        const sampleRoles = [
            {
    ...
    `);
        sampleRoles.forEach(role => {
            roleStmt.run(role.role_id, role.context_id, role.role_type, JSON.stringify(role.permissions), role.role_description, role.hierarchy_level);
        });
