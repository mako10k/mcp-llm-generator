...
describe('Sprint4 Phase 1: ペルソナ管理統合システム', () => {
    ...
    describe('権限・ロール管理', () => {
        ...
        test('ロール階層の作成', () => {
            ...
            const parentResult = personaManager.createRoleHierarchy('', parentRoleId, 'admin_context', 'admin', ['all_permissions'], 'System Administrator');
            expect(parentResult).toBe(true);
            // 子ロール作成
            const childResult = personaManager.createRoleHierarchy(parentRoleId, childRoleId, 'admin_context', 'specialist', // 'manager'を'specialist'に変更
            ['manage_users', 'view_reports'], 'Team Manager');
            expect(childResult).toBe(true);
            // 階層レベル確認
            const childRole = db.prepare('SELECT hierarchy_level FROM persona_roles WHERE role_id = ?').get(childRoleId);
    ...
    describe('タスク委譲システム', () => {
        ...
        test('スマート委譲システム', () => {
            ...
            const delegationId = personaManager.smartDelegateTask('smart_delegator', '機械学習モデルの開発', ['python', 'machine_learning'], { priority: 'high', min_capability_match: 70 });
            expect(delegationId).toBeTruthy();
            // 最適な候補（candidate2）が選ばれたかデータベースで確認
            const delegation = db.prepare('SELECT to_context_id FROM task_delegations WHERE delegation_id = ?').get(delegationId);
