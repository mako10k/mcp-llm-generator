...
import { ParameterFormatter } from '../src/toolcall-emulation/prompt-builders/ParameterFormatter.js';
describe('ParameterFormatter 統合テスト', () => {
    ...
    describe('format メソッド', () => {
        test('必須パラメータのみの場合', () => {
            const result = ParameterFormatter.format(testParameters, ['query'], []);
            expect(result).toContain('**Required**:');
            expect(result).toContain('- `query` (string): 検索クエリ');
            expect(result).not.toContain('**Optional**:');
        });
        test('必須・オプションパラメータ混在の場合', () => {
            const result = ParameterFormatter.format(testParameters, ['query'], ['limit', 'includeMetadata']);
            expect(result).toContain('**Required**:');
            expect(result).toContain('- `query` (string): 検索クエリ');
        ...
        test('オプションパラメータのみの場合', () => {
            const result = ParameterFormatter.format(testParameters, [], ['limit', 'includeMetadata']);
            expect(result).not.toContain('**Required**:');
            expect(result).toContain('**Optional**:');
            expect(result).toContain('- `limit` (number): 結果の最大件数');
        });
        test('パラメータが存在しない場合', () => {
            const result = ParameterFormatter.format(testParameters, [], []);
            expect(result).toBe('');
        });
    });
    ...
    describe('既存ロジックとの互換性テスト', () => {
        test('既存formatParametersと同じ出力を生成することを検証', () => {
            // 既存ロジックをシミュレート
            const existingFormatParameters = (parameters, requiredParams, optionalParams) => {
