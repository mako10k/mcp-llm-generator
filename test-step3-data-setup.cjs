...
try {
  ...
  for (const persona of testPersonas) {
    db.prepare(`
      ...
      (id, name, personality, system_prompt, created_at, updated_at, expires_at)
      VALUES (?, ?, ?, ?, datetime('now'), datetime('now'), datetime('now', '+7 days'))
    `).run(persona.id, persona.name, persona.personality, persona.system_prompt);
    console.log(`✓ ペルソナ作成: ${persona.name} (${persona.id})`);
  }
