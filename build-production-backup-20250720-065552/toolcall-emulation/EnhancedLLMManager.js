...
export class EnhancedLLMManager extends LLMProviderManager {
     ...
     */
    async generateWithStructuredOutput(messages, schema, options = {}) {
     ...
     */
    async emulateToolCalls(userMessage, options) {
        ...
        const result = await this.generateWithStructuredOutput(messages, ToolCallEmulationResponseSchema, structuredOptions);
        // 信頼度フィルタリング
        if (options.confidenceThreshold) {
            result.content.tool_calls = result.content.tool_calls.filter(call => call.confidence >= (options.confidenceThreshold || 0.8));
        }
     ...
     * System Prompt を動的生成
     */
    generateToolCallSystemPrompt(options) {
        const toolDescriptions = options.availableTools.map(tool => `## ${tool.function.name}\n${tool.function.description}\n` +
            `Parameters: ${JSON.stringify(tool.function.parameters, null, 2)}`).join('\n\n');
...
Output Format: Follow the structured schema exactly with no additional properties.`;
    ...
    isRefusal(response) {
        ...
        return refusalPhrases.some(phrase => response.content.toLowerCase().includes(phrase.toLowerCase()));
    }
    /**
     * 信頼度計算
     */
    calculateConfidence(response, parseSuccess) {
     ...
     * フォールバック レスポンス
     */
    fallbackResponse(schema) {
        try {
            const fallback = schema.parse({});
            return {
                content: fallback,
                raw: { content: '{}', model: 'fallback' },
