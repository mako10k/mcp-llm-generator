export class PromptSecurityManager {
     ...
     * プロンプトのセキュリティ検証
     */
    validatePrompt(prompt, securityLevel = 'medium') {
        const attempts = [];
        let riskScore = 0;
        ...
        // 2. 疑わしいキーワード検出
        const suspiciousMatches = this.SUSPICIOUS_KEYWORDS.filter(keyword => prompt.toLowerCase().includes(keyword.toLowerCase()));
     ...
     * プロンプトのサニタイゼーション
     */
    sanitizePrompt(prompt, securityLevel = 'medium') {
     ...
     * 推奨事項の生成
     */
    generateRecommendations(attempts, riskScore) {
     ...
     * セキュリティ設定の動的調整
     */
    adjustSecurityLevel(currentLevel, recentAttempts) {
