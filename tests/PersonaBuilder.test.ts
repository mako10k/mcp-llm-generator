...
describe('PersonaBuilder 統合テスト', () => {
  // テスト用のペルソナデータ
  const testPersona: PersonaCapabilities = {
  ...
  describe('build メソッド', () => {
    ...
    test('一部の情報が欠けている場合', () => {
      const incompletePersona: PersonaCapabilities = {
        expertise: ['AI/ML'],
    ...
    test('すべての情報が欠けている場合', () => {
      const emptyPersona: PersonaCapabilities = {};
    ...
    test('ペルソナがnullまたはundefinedの場合', () => {
      const contextWithoutPersona: PromptContext = {
        availableTools: []
        // persona: undefined
      };
  ...
  describe('配列処理のテスト', () => {
    test('空の配列が適切に処理される', () => {
      const personaWithEmptyArrays: PersonaCapabilities = {
        expertise: [],
        tools: [],
        restrictions: []
      };
    ...
    test('単一要素の配列が適切に処理される', () => {
      const personaWithSingleItems: PersonaCapabilities = {
  ...
  describe('既存ロジックとの互換性テスト', () => {
    test('既存generatePersonaSectionと同じ出力を生成することを検証', () => {
      // 既存ロジックをシミュレート
      const existingGeneratePersonaSection = (persona: PersonaCapabilities): string => {
...
Adapt your tool selection and reasoning to align with these persona characteristics.`;
    ...
    test('複数の代表的なペルソナパターンでの互換性確認', () => {
      const testCases: PersonaCapabilities[] = [
        // 完全なペルソナ
        {
