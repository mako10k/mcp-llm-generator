...
export class GoogleSearchFunction {
   ...
   */
  async executeSearch(params: GoogleSearchParams): Promise<GoogleSearchResult> {
   ...
   */
  private async generateMockSearchResults(config: any): Promise<GoogleSearchResult> {
    // モック検索結果
    const mockResults = [
      {
  ...
  private generateBriefSummary(snippets: string, titles: string[]): string {
    const firstTitle = titles[0] || '検索結果';
    return `${firstTitle}などの情報が見つかりました。${snippets.substring(0, 100)}...`;
  }

  private generateDetailedSummary(snippets: string, titles: string[]): string {
    return `検索により以下の情報が見つかりました：\n\n主要な結果：\n${titles.slice(0, 5).map((t, i) => `${i + 1}. ${t}`).join('\n')}\n\n概要：\n${snippets.substring(0, 300)}...`;
  }

  private extractKeyPoints(snippets: string): string[] {
