...
export class ResponseParser {
   ...
   */
  async parseToolCallResponse(
    rawContent: string,
    requestId?: string
  ): Promise<ParseResult<ToolCallEmulationResponse>> {
  ...
  private validateSchema<T>(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        const errors: ParseError[] = error.errors.map(err => ({
          type: 'schema',
          field: err.path.join('.'),
          message: err.message,
          severity: 'error'
   ...
   * ToolCall固有の検証
   */
  private validateToolCalls(response: ToolCallEmulationResponse): ValidationReport {
  ...
  private calculateConfidence(
    response: ToolCallEmulationResponse,
    ...
    if (response.tool_calls.length > 0) {
      const avgToolConfidence = response.tool_calls.reduce(
   ...
   * 検証スコア計算
   */
  private calculateValidationScore(errors: ParseError[], warnings: ParseError[]): number {
